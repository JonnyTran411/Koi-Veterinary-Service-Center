...
const DoctorAndDateForm = ({ initialData, setFormInstance, onSubmit }) => {
  ...
  // Xử lý submit form, chuyển đổi giá trị moment sang chuỗi ngày
  const handleSubmit = (values) => {
    const { selectedDate, ...rest } = values;
    // Chuyển đổi selectedDate từ moment sang chuỗi
    const formattedDate = selectedDate
      ? selectedDate.format("DD/MM/YYYY")
      : null;
    // Gửi dữ liệu với ngày đã được chuyển đổi
    onSubmit({ ...rest, selectedDate: formattedDate });
  };
  ...
  return (
    <Form
      ...
      onFinish={handleSubmit} // Thay đổi xử lý submit
